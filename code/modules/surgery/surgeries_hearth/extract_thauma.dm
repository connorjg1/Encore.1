/datum/surgery/extract_thauma
	steps = list(
		/datum/surgery_step/incise,
		/datum/surgery_step/clamp,
		/datum/surgery_step/retract,
		/datum/surgery_step/saw,
		/datum/surgery_step/extract_thauma,
...
/datum/surgery_step/extract_thauma/validate_target(mob/user, mob/living/target, target_zone, datum/intent/intent)
	. = ..()
	if(target.stat == DEAD)
		to_chat(user, "They're dead!")
		return FALSE

/datum/surgery_step/extract_thauma/preop(mob/user, mob/living/target, target_zone, obj/item/tool, datum/intent/intent)
...
/datum/surgery_step/extract_thauma/success(mob/user, mob/living/target, target_zone, obj/item/tool, datum/intent/intent)
	...
	if(target.has_status_effect(/datum/status_effect/debuff/devitalised))
		...
		display_results(user, target, span_notice("You extract a single dose of thauma from [target]'s heart."),
			"[user] extracts thauma from [target]'s innards.",
			"[user] extracts thauma from [target]'s innards.")
		new /obj/item/reagent_containers/thauma(target.loc)
