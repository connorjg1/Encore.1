	...
	if(buckled)
		...
		if(stat == DEAD && !M.cmode)
			...
			if(possible_len)
				...
				if(possible_len > 1)
					var/input = input(M, "Which surgery step do you want to perform?", "PHYSIC", ) as null|anything in possible_steps
					if(input)
	...
	if(stat == DEAD && !user.cmode)
		...
		if(possible_len)
			...
			if(length(possible_steps) > 1)
				var/input = input(user, "Which surgery step do you want to perform?", "PHYSIC", ) as null|anything in possible_steps
				if(input)
					done_step = possible_steps[input]
			else
				done_step = possible_steps[possible_steps[1]]
