...
/obj/structure/far_travel/MouseDrop_T(atom/dropping, mob/user)
	...
	user.visible_message("<span class='warning'>[user] [departing_mob == user ? "is trying to depart from Domotan Island!" : "is trying to send [departing_mob] away!"]</span>", "<span class='notice'>You [departing_mob == user ? "are trying to depart from Domotan Island." : "are trying to send [departing_mob] away."]</span>")
	in_use = TRUE
	...
	if(departing_mob.stat == DEAD)
		...
		departing_mob.visible_message("<span class='notice'>[departing_mob == user ? "Out of their own volition, " : "Ushered by [user], "][departing_mob] leaves Domotan Island.</span>")
	qdel(departing_mob)
